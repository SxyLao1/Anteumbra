...
class QuarantineHandlerPlugin(Plugin):
    ...
    def activate(self, config: Dict[str, Any]) -> None:
        try:
            ...
            self._batch_threshold = 50
        self._batch_state = {}
        self._logger.info("QuarantineHandler: 已激活 (batch_threshold=%d)", self._batch_threshold)
    ...
    def on_event(self, event: DomainEvent) -> Optional[List[DomainEvent]]:
        ...
        if result is not None:
            ...
            batch_site_name = str(
                site_name or ("Legacy / unassigned" if batch_site_id == "legacy" else batch_site_id)
            ).strip()
