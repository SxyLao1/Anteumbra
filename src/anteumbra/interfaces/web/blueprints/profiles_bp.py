    ...
    try:
        ...
        if q:
            all_profiles = [
                ...
                if q in p.profile_id.lower()
                or q in p.ua_fingerprint.lower()
                or q in p.tool_signature.lower()
                or any(q in ip for ip in p.ip_pool)
            ]
...
def profile_detail_page(profile_id):
    ...
    try:
        ...
        try:
            ...
            runtime = get_runtime()
            all_reg = runtime.registry.get_all(include_deleted=True, site_id=profile.site_id)
            qmap = {}
            for q in runtime.quarantine.list_records(
