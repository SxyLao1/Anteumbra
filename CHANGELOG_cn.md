...
### 修复
- 中文目录编译后设置页与插件面板直接返回错误页：八条翻译的映射占位符漏写了转换字符
  （例如原文是 '%(count)s'，译文写成 '%(count)'），Python 的 % 格式化于是把紧随其后的
  字符当成转换类型并抛出 "unsupported format character"（0x9879 就是设置页变更计数里的
  那个字）。受影响的条目——隔离确认、删除规则确认、变更计数——已修复；并新增守卫测试，
  逐条比对译文与原文的占位符并扫描已编译目录，因为"键是否存在"式的测试抓不到这类问题。
## [1.0.39] - 2026-09-14
