...
<img src="https://img.shields.io/badge/version-1.0.40-blue?style=flat-square" alt="Version">
...
- 插件管理器与 WAF / 事件源扩展接口
- 多站点操作界面：导航站点切换器、按站点总览，以及按站点区分的检出、隔离与文件类聚视图
- 内存马检测、取证与处置：针对只存在于容器内存、磁盘上没有文件的 webshell，并对字节码增强型工具明确标注检测边界
- MCP 服务器（`anteumbra mcp serve`）与随包发布的 Agent Skill，让本地 AI 助手可以配置并验证部署
- 高级配置编辑器：可搜索的 TOML 结构树、校验、时间戳备份与版本回滚
- 设置页里的插件配置：每个字段显示生效值与来源
...
<div align="center">
  <sub>Anteumbra v1.0.40 · MIT License</sub>
</div>
