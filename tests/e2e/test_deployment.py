...
class TestVersionSource:
    ...
    def test_generated_reports_use_runtime_version(self):
        """Generated report footers must not expose fixed legacy versions."""
        project_root = Path(__file__).parent.parent.parent
        templates = (
            project_root / "src" / "anteumbra" / "interfaces" / "web" / "templates" / "admin"
        )
        report_paths = (
            templates / "profile_report.html",
            templates / "scanner_report.html",
        )
        ...
        for report_path in report_paths:
            ...
            output_lines = (
                ...
                if "Generated by Anteumbra" in line or "Manual Scanner Report" in line
            )
            assert "anteumbra_version" in source
            assert not any(re.search(r"v\d+\.\d+\.\d+", line) for line in output_lines)
